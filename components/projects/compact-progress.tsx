...
import { PROGRESS_CAP_PERCENTAGE, PROGRESS_UPDATE_INTERVAL_MS } from "@/lib/constants"
import { estimateAssemblyAITime } from "@/lib/processing-time-estimator"

type JobStatusStep = "running" | "completed" | "failed"

interface JobStatus {
  transcription: JobStatusStep
  keyMoments: JobStatusStep
  summary: JobStatusStep
  social: JobStatusStep
  titles: JobStatusStep
  hashtags: JobStatusStep
  youtubeTimestamps: JobStatusStep
}
...
export function CompactProgress({ jobStatus, fileDuration, createdAt }: CompactProgressProps) {
  ...
  }, [jobStatus, isTranscribing, createdAt, fileDuration, completedSteps, totalSteps])

  if (!jobStatus) {
    return null
  }

  const statusText = isTranscribing ? "🎙️ Transcribing..." : "✨ Generating content..."

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Badge className="text-xs font-semibold bg-emerald-100 text-emerald-700 border-emerald-200">{statusText}</Badge>
